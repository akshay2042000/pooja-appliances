...
export default function BillList() {
	...
	const { billList, billListLoading, billListError } = useSelector(
	...
	const bills = billList.map((bill) => {
		const obj = {
			...
			totalTaxable: bill.invoiceData.items.reduce((acc, item) => {
				return parseFloat(
					parseFloat(acc) + parseFloat(item.taxableValue)
				).toFixed(2);
			}, 0),
			totalGst: bill.invoiceData.items.reduce((acc, item) => {
				return parseFloat(
					parseFloat(acc) +
			...
			invoiceTotal: bill.invoiceData.items.reduce((acc, item) => {
				return parseFloat(parseFloat(acc) + parseFloat(item.subtotal)).toFixed(
					2
				);
			}, 0),
			order: bill.order,
	...
	const columns = [
		{
			...
			renderCell: (params) => {
				return (
					<Typography
						title={params.row.invoiceNumber}
						variant="body2"
		...
		},
		{
			...
			renderCell: (params) => {
				return (
					<Typography
						title={moment(params.row.date).format("MMM Do YY, h:mm a")}
						variant="body2"
						sx={{ textOverflow: "ellipsis", overflow: "hidden" }}
					>
		...
		},
		{
			...
			renderCell: (params) => {
				return (
					<Typography
						title={params.row.order.orderId}
						variant="body2"
						sx={{ textOverflow: "ellipsis", overflow: "hidden" }}
					>
				...
				return (
					...
					>
						<Button
							variant="contained"
							color="secondary"
							onClick={(e) => download(params.row.downloadLink)}
						>
							Download
						</Button>
						<IconButton
